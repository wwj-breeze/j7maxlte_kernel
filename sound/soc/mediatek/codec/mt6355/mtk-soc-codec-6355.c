...
#include "../../../../drivers/misc/mediatek/include/mt-plat/mtk_boot.h"
...
static unsigned int mUseExtCLKSource;
...
bool UseExtCLKSource(void)
{
	return mUseExtCLKSource != 0;
}
...
static void SetDcCompenSation(bool enable)
{
	...
	pr_warn("%s() mAudio_Ana_Volume L = %d, mAudio_Ana_Volume R = %d\n",
		__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL],
	...
	pr_warn("%s times = %d, lch_value = 0x%x, rch_value = 0x%x\n", __func__, times, lch_value, rch_value);
	if (enable) {
...
static int Headset_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_warn("%s() set mAudio_Ana_Volume L = %d, mAudio_Ana_Volume R = %d\n",
		__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL],
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR]);
...
static int Audio_HyBridNLE_SetGain_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (nle_channel == AUDIO_NLE_CHANNEL_R) {
		...
		pr_warn("%s() set mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR] =%d\n",
			__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR]);
		...
		pr_warn("%s() set mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL] =%d\n",
			__func__, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL]);
	} else
